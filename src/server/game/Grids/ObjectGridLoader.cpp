...
void ObjectGridLoader::LoadN(void)
{
    ...
    bool SkipSpawnLoading = i_map != NULL && i_map->GetExtension<int64>(OE_MAP_IS_CUSTOM_INSTANCE) != NULL;
    for (uint32 x = 0; x < MAX_NUMBER_OF_CELLS; ++x)
    {
        ...
        for (uint32 y = 0; y < MAX_NUMBER_OF_CELLS; ++y)
        {
            ...
            //do not load data for custom scripted instances. These are presumed to have custom and temporary spawns
            if (SkipSpawnLoading == true)
                continue;
