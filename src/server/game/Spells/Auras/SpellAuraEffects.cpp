...
AuraEffect::AuraEffect(int THISWILLCRASHTHESERVER, int IAGREEIHAVENOIDEAWHATIMDOING, Aura *a, const SpellInfo *sp, uint8 effIndex):
    m_base(a), m_spellInfo(sp), m_baseAmount(m_spellInfo->Effects[0].BasePoints), m_effIndex(effIndex)
{   
    _amount = 0;
    m_spellmod = nullptr;
    _periodicTimer = 0;
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (crit)
    {
        CritDmg = Unit::SpellCriticalDamageBonus(caster, m_spellInfo, damage, target);
        damage += CritDmg;
    }


    SpellSchoolMask UsedSchoolMask = GetSpellInfo()->GetSchoolMask();
    if(caster != NULL)
        if (uint32 *ChangeSchools = caster->GetExtension<uint32>(OE_PLAYER_CHANGE_DAMGE_SCHOOL_MASK))
            UsedSchoolMask = SpellSchoolMask( UsedSchoolMask | ChangeSchools[0]);

    // Calculate armor mitigation
    if (Unit::IsDamageReducedByArmor(UsedSchoolMask, GetSpellInfo()))
    {
    ...
    DamageInfo damageInfo(caster, target, damage, GetSpellInfo(), UsedSchoolMask, DOT, BASE_ATTACK);
    ...
    CP_DMG_DONE_RECEIVED CBD(caster, target, damage - CritDmg, GetTotalTicks(), GetSpellInfo(), MAX_ATTACK, CritDmg);
    TriggerCallbackFunctions(CALLBACK_TYPE_DMG_DONE, &CBD);
    if (caster)
        caster->TriggerCallbackFunc(CALLBACK_TYPE_DMG_DONE, &CBD);
    if (target)
        target->TriggerCallbackFunc(CALLBACK_TYPE_DMG_RECEIVED, &CBD);
    TriggerCallbackFunctions(CALLBACK_TYPE_DMG_RECEIVED, &CBD);
    damage = CBD.GetDamage();

    SpellPeriodicAuraLogInfo pInfo(this, damage, overkill, absorb, resist, 0.0f, crit, UsedSchoolMask);
    target->SendPeriodicAuraLog(&pInfo);

    Unit::DealDamage(caster, target, damage, &cleanDamage, DOT, UsedSchoolMask, GetSpellInfo(), true);
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    healInfo.TickCount = GetTotalTicks();
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    SpellPeriodicAuraLogInfo pInfo(this, heal, heal - healInfo.GetEffectiveHeal(), healInfo.GetAbsorb(), 0, 0.0f, crit, GetSpellInfo()->GetSchoolMask());
    target->SendPeriodicAuraLog(&pInfo);
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    SpellPeriodicAuraLogInfo pInfo(this, drainedAmount, 0, 0, 0, gainMultiplier, false, GetSpellInfo()->GetSchoolMask());
    target->SendPeriodicAuraLog(&pInfo);
...
void AuraEffect::HandleObsModPowerAuraTick(Unit* target, Unit* caster) const
{
    ...
    SpellPeriodicAuraLogInfo pInfo(this, amount, 0, 0, 0, 0.0f, false, GetSpellInfo()->GetSchoolMask());
    target->SendPeriodicAuraLog(&pInfo);
...
void AuraEffect::HandlePeriodicPowerBurnAuraTick(Unit* target, Unit* caster) const
{
    ...
    int32 TickCount = GetTotalTicks();
    caster->CalculateSpellDamageTaken(&damageInfo, int32(gain * dmgMultiplier), spellProto, BASE_ATTACK, false, TickCount);
...
void AuraEffect::HandleProcTriggerSpellAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    if (SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(triggerSpellId))
    {
        if (triggerSpellId != 0) TC_LOG_DEBUG("spells", "AuraEffect::HandleProcTriggerSpellAuraProc: Triggering spell %u from aura %u proc", triggeredSpellInfo->Id, GetId());
        triggerCaster->CastSpell(triggerTarget, triggeredSpellInfo->Id, this);
    }
    else {
        if (triggerSpellId != 0) TC_LOG_ERROR("spells", "AuraEffect::HandleProcTriggerSpellAuraProc: Could not trigger spell %u from aura %u proc, because the spell does not have an entry in Spell.dbc.", triggerSpellId, GetId()); }
}
...
void AuraEffect::HandleProcTriggerDamageAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    int32 TickCount = GetTotalTicks();
    target->CalculateSpellDamageTaken(&damageInfo, damage, GetSpellInfo(), BASE_ATTACK, false, TickCount);
    Unit::DealDamageMods(damageInfo.target, damageInfo.damage, &damageInfo.absorb);
