...
void Loot::generateMoneyLoot(uint32 minAmount, uint32 maxAmount, Player* lootOwner)
{
    if (maxAmount > 0)
    {
        ...
        CP_INT32_PARAM lgcb((int32*)&gold, lootOwner);
        TriggerCallbackFunctions(CALLBACK_TYPE_LOOT_GOLD_GENERATED, &lgcb);
        if(lootOwner)
            lootOwner->TriggerCallbackFunc(CALLBACK_TYPE_LOOT_GOLD_GENERATED, &lgcb);
    }
}
