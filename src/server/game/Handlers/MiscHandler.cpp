...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recvData)
{
    ...
    _player->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_GOSSIP_SELECT, &CP_GOSSIP_SELECT(_player, &guid, gossipListId, menuId));
...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    for (WhoListPlayerInfo const& target : whoList)
    {
        ...
        //stop showing fake players for those who played some time
        if (GetPlayer()->GetTotalPlayedTime() > 4 * HOUR && sWorld->GetOnlinePlayer(target.GetGuid()) == NULL)
            continue;
...
void WorldSession::HandleInspectOpcode(WorldPacket& recvData)
{
    ...
    *GetPlayer()->GetCreateExtension<uint64>(OE_PLAYER_LAST_INSPECTED) = guid.GetRawValue();
