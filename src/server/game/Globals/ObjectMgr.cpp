...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    //                                                0       1       2      3       4       5      6
    QueryResult result = WorldDatabase.Query("SELECT guid, path_id, mount, bytes1, bytes2, emote, auras, scale FROM creature_addon");
...
void ObjectMgr::LoadGameObjectAddons()
{
    ...
    //                                               0     1                 2                 3                 4                 5                 6
    QueryResult result = WorldDatabase.Query("SELECT guid, parent_rotation0, parent_rotation1, parent_rotation2, parent_rotation3, invisibilityType, invisibilityValue, scale FROM gameobject_addon");
    ...
    do
    {
        ...
        gameObjectAddon.Scale = fields[7].GetFloat();
...
void ObjectMgr::UpdateCreatureAddon(ObjectGuid::LowType lowguid, CreatureAddon const *NewValues)
{
    CreatureAddon &ca = _creatureAddonStore[lowguid];
    ca = *NewValues;
    //update DB also
    std::ostringstream AuraString;
    for (auto itr = ca.auras.begin(); itr != ca.auras.end(); itr++)
        AuraString << (*itr) << " ";
    WorldDatabase.PQuery("replace into creature_addon (guid, path_id, mount, bytes1, bytes2, emote, auras, scale) values (%d,%d,%d,%d,%d,%d,'%s',%f)",
        lowguid, ca.path_id, ca.mount, ca.bytes1, ca.bytes2, ca.emote, AuraString.str().c_str(), ca.Scale);

}

void ObjectMgr::UpdateGameObjectAddon(ObjectGuid::LowType lowguid, GameObjectAddon const *NewValues)
{
    GameObjectAddon &ga = _gameObjectAddonStore[lowguid];
    ga = *NewValues;
    //update DB also
    WorldDatabase.PQuery("replace into gameobject_addon (guid, parent_rotation0, parent_rotation1, parent_rotation2, parent_rotation3, invisibilityType, invisibilityValue, scale) values (%d,%f,%f,%f,%f,%d,%d,%f)",
        lowguid, ga.ParentRotation.x, ga.ParentRotation.y, ga.ParentRotation.z, ga.ParentRotation.w, ga.invisibilityType, ga.InvisibilityValue, ga.Scale);
}
