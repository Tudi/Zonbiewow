...
void Creature::Update(uint32 diff)
{
                ...
                else                                // the master is dead
                {
                    ...
                    else
                    {
                        ...
                        if (baseRespawnTime <= std::numeric_limits<time_t>::max() - offset)
                            ...
                            m_respawnTime = GameTime::GetGameTime() + MONTH * 10;
                    }
    ...
    //check if any server side scripting needs this event. Only call it once.
    if (GetExtension<uint32>(OE_CREATURE_SPAWN_STAMP) == NULL)
    {
        *GetCreateExtension<uint32>(OE_CREATURE_SPAWN_STAMP) = GameTime::GetGameTimeMS();
        //this should be delayed enough so that aura casts will not fail to show client side
        TriggerCallbackFunctions(CALLBACK_TYPE_CREATURE_SPAWN, &CP_CREATURE_SPAWN(this, GetMap()));
    }
    TriggerCallbackFunc(CALLBACK_TYPE_CREATURE_UPDATE, this);
}
    ...
    if (s == JUST_DIED)
    {
        ...
        if (m_respawnCompatibilityMode)
        {
            if (IsDungeonBoss() && !m_respawnDelay)
                m_respawnTime = GameTime::GetGameTime() + MONTH * 10; // never respawn in this instance
            else
