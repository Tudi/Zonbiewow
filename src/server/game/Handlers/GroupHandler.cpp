...
void WorldSession::HandleGroupInviteOpcode(WorldPacket& recvData)
{
    ...
    CP_ASK_DISABLED_FEATURE CP(GetPlayer());
    GetPlayer()->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_GROUP_INVITE, &CP);
    ...
    CP_GROUP_INVITE CPW(GetPlayer(), membername.c_str());
    TriggerCallbackFunctions(CALLBACK_TYPE_GROUP_INVITE, &CPW);
    if (CPW.DenyDefaultParsing == 1)
        return;
...
void WorldSession::HandleGroupAcceptOpcode(WorldPacket& recvData)
{
    ...
    CP_ASK_DISABLED_FEATURE CP(GetPlayer());
    GetPlayer()->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_GROUP_ACCEPT, &CP);
    if (CP.DenyDefaultParsing == 1)
        return;
