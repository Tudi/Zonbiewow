...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    //if some addon wants to keep this player silent
    CP_CHAT_RECEIVED cr(GetPlayer(), type, &channel, &msg, &to);
    TriggerCallbackFunctions(CALLBACK_TYPE_CHAT_RECEIVED, &cr);
    if (cr.DenyDefaultParsing)
        return;
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_WHISPER:
        {
            CP_WHISPER_FROM_CLIENT CPW(GetPlayer(), msg.c_str(), to.c_str());
            TriggerCallbackFunctions(CALLBACK_TYPE_WHISPER_NOCHECKS, &CPW);
