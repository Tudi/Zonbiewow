...
, m_spellValue(new SpellValue(m_spellInfo)), _spellEvent(nullptr)
{
    ...
    if (playerCaster != NULL)
    {
        int64 const *tTriggerFlags = playerCaster->GetExtension<int64>(OE_PLAYER_CAST_WHILE_MOVE);
        if (tTriggerFlags != NULL && tTriggerFlags[0] > 0)
            _triggeredCastFlags = TriggerCastFlags(uint32(_triggeredCastFlags) | TRIGGERED_IGNORE_MOVE_CHECK);
        tTriggerFlags = playerCaster->GetExtension<int64>(OE_PLAYER_CAST_WHILE_CAST);
        if (tTriggerFlags != NULL && tTriggerFlags[0] > 0 && m_spellInfo->CalcCastTime(this) == 0)
            _triggeredCastFlags = TriggerCastFlags(uint32(_triggeredCastFlags) | TRIGGERED_IGNORE_CAST_IN_PROGRESS);
        tTriggerFlags = playerCaster->GetExtension<int64>(OE_PLAYER_CAST_WHILE_BAD_FACING);
        if (tTriggerFlags != NULL && tTriggerFlags[0] > 0)
            _triggeredCastFlags = TriggerCastFlags(uint32(_triggeredCastFlags) | TRIGGERED_IGNORE_SET_FACING);
    }
    m_CastItem = nullptr;
...
void Spell::TargetInfo::DoDamageAndTriggers(Spell* spell)
{
    ...
    if (_spellHitTarget)
    {
        ...
        if (Player* playerCaster = spell->m_caster->ToPlayer())
        {
            CP_SPELL_CAST cpsc(playerCaster, spell->GetSpellInfo(), spell->m_UniqueTargetInfo.size(), _spellHitTarget, spell, procAttacker, procVictim, procSpellType);
            playerCaster->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_SPELL_POST_CAST, &cpsc);
        }
    }
...
void Spell::prepare(SpellCastTargets const& targets, AuraEffect const* triggeredByAura)
{
    ...
    if ((_triggeredCastFlags & TRIGGERED_IGNORE_CAST_TIME) == 0)
    {
        if (Player* player = m_caster->ToPlayer())
        {
    ...
    else
    {
        if (Unit* unitCaster = m_caster->ToUnit())
        {
            ...
            {
                uint32 InterruptFlags = AURA_INTERRUPT_FLAG_CAST;
                ...
                for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                {
                    if (m_spellInfo->Effects[i].GetUsedTargetObjectType() == TARGET_OBJECT_TYPE_UNIT)
                    {
                        ...
                        InterruptFlags |= AURA_INTERRUPT_FLAG_SPELL_ATTACK;
                        break;
                    }
                }
                if ((GetTriggerFlags() & TRIGGERED_IGNORE_CAST_IN_PROGRESS) == 0)
                {
                    Spell* spell = unitCaster->GetCurrentSpell(CURRENT_CHANNELED_SPELL);
                    if (spell != NULL
                        && spell->getState() == SPELL_STATE_CASTING
                        && (spell->m_spellInfo->ChannelInterruptFlags & InterruptFlags))
                        unitCaster->InterruptNonMeleeSpells(false);
                }
            }
...
void Spell::TakeCastItem()
{
    ...
    if (expendable && withoutCharges)
    {
        ...
        CP_ITEM_USE_DENY IUD(m_caster->ToPlayer(), m_CastItem, NULL);
        m_caster->ToPlayer()->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_ITEM_CONSUME, &IUD);
        if (IUD.DenyDefaultParsing == 0)
...
SpellCastResult Spell::CheckCast(bool strict, uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/)
{
    ...
    if (Unit* unitCaster = m_caster->ToUnit())
    {
        ...
        if ((_triggeredCastFlags & TRIGGERED_IGNORE_MOVE_CHECK) == 0)
        {
...
SpellCastResult Spell::CheckRange(bool strict) const
{
    ...
    if (target && target != m_caster)
    {
        ...
        if (m_caster->GetTypeId() == TYPEID_PLAYER &&
            ...
            && ( GetTriggerFlags() & TRIGGERED_IGNORE_SET_FACING ) == 0
            )
