...
bool Player::Ironman() {
    return (IsIronmanPlr(this) && !IsHardcorePlr(this));
}

bool Player::Hardcore() {
    return IsHardcorePlr(this);
}

void Player::AddCredits(uint32 reward)
{
    if (GetSession()) {
        uint32 AccountId = GetSession()->GetAccountId();
        char Query[5000];
        sprintf_s(Query, sizeof(Query), "INSERT INTO account_credits (`AccId`,`reward`) VALUES ( %d, %d ) ON DUPLICATE KEY UPDATE reward = reward + %d", AccountId, reward, reward);
...
void Player::Update(uint32 p_time)
{
    ...
    //use this with great care. Maybe later send update time also
    this->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_PERIODIC_TICK, this);
    //this is a dirty way to fix some delayed spell casting :(
    if (IsAlive() == true && IsNonMeleeSpellCast(true) == false)
    {
        uint32 *DamageToCast = GetExtension<uint32>(OE_PLAYER_AOE_DMG);
        if (DamageToCast != NULL && DamageToCast[0] > 0)
        {
            Unit *Target = GetSelectedUnit();
            SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(6295);
            Spell* spell = new Spell(this, spellInfo, (TriggerCastFlags)(TRIGGERED_FULL_MASK | TRIGGERED_IGNORE_CAST_TIME | TRIGGERED_IGNORE_MOVE_CHECK));
            spell->m_targets.SetUnitTarget(Target);
            if (Target != NULL)
                spell->m_targets.SetDst(Target->GetPosition());
            spell->SetSpellValue(SPELLVALUE_BASE_POINT0, int32(DamageToCast[0]));
            spell->cast(true);
...
bool Player::AddSpell(uint32 spellId, bool active, bool learning, bool dependent, bool disabled, bool loading /*= false*/, uint32 fromSkill /*= 0*/)
{
    ...
    if (!disabled_case) // skip new spell adding if spell already known (disabled spells case)
    {
        ...
        else if (uint32 prev_spell = sSpellMgr->GetPrevSpellInChain(spellId) && loading == false)
        {
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
    ...
    this->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_REVIVE, this);
...
void Player::ApplyRatingMod(CombatRating combatRating, int32 value, bool apply)
{
    ...
    if (value > 0x01FFF)
    {
        printf("Nerfed combat rating %d value %d to %d in fear of overflow\n", combatRating, value, 0x01FFF);
        value = 0x01FFF;
    }
    else if(value <= -0x01FFF)
    {
        value = 0x01FFF;
    }
#endif
...
bool Player::RewardHonor(Unit* victim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    // Back to int now
    honor = int32(honor_f);
    TriggerCallbackFunctions(CALLBACK_TYPE_PLAYER_HONOR_CALC, &CP_HONOR_RECEIVE(this, victim, &honor));
...
void Player::_ApplyItemMods(Item* item, uint8 slot, bool apply, bool updateItemAuras /*= true*/)
{
    ...
    //we also want to apply item stats on login, revive ....
    TriggerCallbackFunctions(CALLBACK_TYPE_PLAYER_ITEM_EQUIP, &CP_ITEM_EQUIPPED(this, item, apply));
...
void Player::_ApplyWeaponDamage(uint8 slot, ItemTemplate const* proto, bool apply)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
    {
        ...
        {
//            if(maxDamage>0)
//                this->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_WEAPON_DAMAGE_CALC, &CP_WEAPON_DMG_CALC(this, proto, attType, &minDamage, &maxDamage, i));
...
void Player::_RemoveAllItemMods()
{
    ...
    for (uint8 i = 0; i < INVENTORY_SLOT_BAG_END; ++i)
    {
        if (m_items[i])
        {
            ...
            TriggerCallbackFunctions(CALLBACK_TYPE_PLAYER_ITEM_EQUIP, &CP_ITEM_EQUIPPED(this, m_items[i], false));
        }
    }
...
void Player::_ApplyAllItemMods()
{
    ...
    for (uint8 i = 0; i < INVENTORY_SLOT_BAG_END; ++i)
    {
        if (m_items[i])
        {
            ...
            //we also want to apply item stats on login, revive ....
            TriggerCallbackFunctions(CALLBACK_TYPE_PLAYER_ITEM_EQUIP, &CP_ITEM_EQUIPPED(this, m_items[i], true));
...
void Player::SendLoot(ObjectGuid guid, LootType loot_type)
{
    ...
    if (FindMap() && GetMap()->GetExtension<int64>(OE_MAP_IS_CUSTOM_INSTANCE))
    {
        ...
        if (go->getLootState() == GO_READY)
        {
            ...
            if (go->GetLootMode() > 0)
                if (GameObjectTemplateAddon const* addon = go->GetTemplateAddon())
                    loot->generateMoneyLoot(addon->mingold, addon->maxgold,this);
    ...
    else if (guid.IsCorpse())                          // remove insignia
    {
        ...
        if (loot->loot_type == LOOT_NONE)
        {
            ...
            CP_INT32_PARAM lgcb((int32*)&bones->loot.gold, this);
            TriggerCallbackFunctions(CALLBACK_TYPE_LOOT_GOLD_GENERATED, &lgcb);
            TriggerCallbackFunc(CALLBACK_TYPE_LOOT_GOLD_GENERATED, &lgcb);
        }
        ...
        if (loot_type == LOOT_PICKPOCKETING)
        {
            if (loot->loot_type != LOOT_PICKPOCKETING)
            {
                if (creature->CanGeneratePickPocketLoot())
                {
                    ...
                    CP_INT32_PARAM lgcb((int32*)&loot->gold, this);
                    TriggerCallbackFunctions(CALLBACK_TYPE_LOOT_GOLD_GENERATED, &lgcb);
                    TriggerCallbackFunc(CALLBACK_TYPE_LOOT_GOLD_GENERATED, &lgcb);
                    permission = OWNER_PERMISSION;
                }
    ...
    {
        TriggerCallbackFunctions(CALLBACK_TYPE_LOOT_SEND, &CP_LOOT_SEND(this, &guid, loot));
    ...
    if (pItem)
    {
        ...
        if (pProto)
        {
            ...
            if (eslot == EQUIPMENT_SLOT_OFFHAND)
            {
                ...
                {
                    if (!CanDualWield() && not_loading == false)
                        return EQUIP_ERR_CANT_DUAL_WIELD;
                }
                else if (type == INVTYPE_2HWEAPON)
                {
                    if ((!CanDualWield() || !CanTitanGrip()) && not_loading == false)
                        return EQUIP_ERR_CANT_DUAL_WIELD;
                }
            ...
            if (type == INVTYPE_2HWEAPON)
            {
                if (eslot == EQUIPMENT_SLOT_OFFHAND)
                {
                    if (!CanTitanGrip() && not_loading == false)
                        return EQUIP_ERR_ITEM_CANT_BE_EQUIPPED;
                }
...
Item* Player::StoreItem(ItemPosCountVec const& dest, Item* pItem, bool update)
{
    ...
    //trigger scripts that we are storing an item. This does not look from where the item is comming from. Could be a duplicate event if used for achievements
    TriggerCallbackFunctions(CALLBACK_TYPE_PLAYER_ITEM_STORED, &CP_ITEM_STORED(pItem->GetTemplate(), this, pItem));
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
        ...
        for (uint32 i = 0; i < quest->GetRewItemsCount(); ++i)
        {
            if (uint32 itemId = quest->RewardItemId[i])
            {
                ...
                if (CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, itemId, quest->RewardItemIdCount[i]) == EQUIP_ERR_OK)
                {
                    ...
                    TriggerCallbackFunc(CALLBACK_TYPE_LOOT_ITEM, &CP_ITEM_LOOTED(this, item));
    ...
    CP_INT32_PARAM lgcb(&moneyRew, this);
    TriggerCallbackFunctions(CALLBACK_TYPE_LOOT_GOLD_GENERATED, &lgcb);
    TriggerCallbackFunc(CALLBACK_TYPE_LOOT_GOLD_GENERATED, &lgcb);
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        {
            //try to store it in inventory
            ItemPosCountVec off_dest;
            Item* offItem = problematicItems.front();
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    else
    {
        ...
        if (FindMap() != NULL && GetMap()->GetExtension<int64>(OE_MAP_IS_CUSTOM_INSTANCE) != NULL)
        {
            stmt->setUInt16(index++, m_homebindMapId);
            stmt->setUInt32(index++, 0);
            stmt->setUInt8(index++, DUNGEON_DIFFICULTY_NORMAL);
            stmt->setFloat(index++, m_homebindX);
            stmt->setFloat(index++, m_homebindY);
            stmt->setFloat(index++, m_homebindZ);
            stmt->setFloat(index++, finiteAlways(GetOrientation()));
        }
...
{
    //if we scripted resets, do it with this instead making lots of changes
    TriggerCallbackFunctions(CALLBACK_TYPE_RESET_ALL_INSTANCES, &CP_ASK_DISABLED_FEATURE(this));
...
void Player::BroadcastMessage(char const* fmt, ...)
{
    if (GetSession() == NULL)
        return;
    ...
    va_start(l, fmt);
    char Message[4*1024];
    vsnprintf(Message, sizeof(Message), fmt, l);
    va_end(l);

    ChatHandler(GetSession()).SendSysMessage(Message);
}
...
void Player::ApplySpellMod(uint32 spellId, SpellModOp op, T& basevalue, Spell* spell /*= nullptr*/)
{
    ...
    int32 tbase = basevalue;
    TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_SPELL_MOD_APPLY, &CP_SPELL_MOD_APPLY(this, spellInfo, op, tbase, &totalflat, &totalmul));
...
bool Player::isFreeArenaVendor(Creature* pVendor) {
    if (getLevel() < 80)
    {
        pVendor->Whisper(std::string("You may get your free arena items only when you reach level 80!"), LANG_UNIVERSAL, this);
        return false;
    }

    bool isFreeItem = false;
    if (pVendor->GetEntry() == 130001 || pVendor->GetEntry() == 130004)
    {
        isFreeItem = true;
    }

    if (IsIronmanPlr(this) || IsHardcorePlr(this))
    {
        if (pVendor->GetEntry() == 130002 || pVendor->GetEntry() == 130005)
            isFreeItem = true;
    }

    if (IsHardcorePlr(this))
    {
        if (pVendor->GetEntry() == 130003 || pVendor->GetEntry() == 130006)
            isFreeItem = true;
    }
    return isFreeItem;
}
...
inline bool Player::_StoreOrEquipNewItem(uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot, int32 price, ItemTemplate const* pProto, Creature* pVendor, VendorItem const* crItem, bool bStore)
{
    ...
    if (crItem->ExtendedCost && !isFreeArenaVendor(pVendor))                            // case for new honor system
    {
    ...
    if (it)
    {
        ...
        if (pProto->Flags & ITEM_FLAG_ITEM_PURCHASE_RECORD && crItem->ExtendedCost && pProto->GetMaxStackSize() == 1 && !isFreeArenaVendor(pVendor))
        {
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    //scripting
    CP_VENDOR_BUY_ITEM CP(this, creature, item);
    TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_VENDOR_BUY_ITEM, &CP);
    if (CP.DenyDefaultParsing == 1)
        return false;
    ...
    if (crItem->ExtendedCost && !isFreeArenaVendor(creature))
    {
        ItemExtendedCostEntry const* iece = sItemExtendedCostStore.LookupEntry(crItem->ExtendedCost);
        if (!iece)
        {
...
void Player::UpdateHomebindTime(uint32 time)
{
    ...
    if (m_InstanceValid || IsGameMaster() || (FindMap() && GetMap()->GetExtension<int64>(OE_MAP_IS_CUSTOM_INSTANCE)))
    {
...
void Player::ResetSpells(bool myClassOnly)
{
    ...
    if (myClassOnly)
    {
        ...
        PlayerSpellMap tmap = smap;
        for (PlayerSpellMap::const_iterator iter = tmap.begin(); iter != tmap.end(); ++iter)
        {
...
uint32 Player::GetRuneBaseCooldown(uint8 index)
{
    ...
    int32 cooldown = RUNE_BASE_COOLDOWN;
...
void Player::AutoStoreLoot(uint8 bag, uint8 slot, uint32 loot_id, LootStore const& store, bool broadcast)
{
    ...
    for (uint32 i = 0; i < max_slot; ++i)
    {
        ...
        TriggerCallbackFunc(CALLBACK_TYPE_LOOT_ITEM, &CP_ITEM_LOOTED(this, pItem));
...
void Player::StoreLootItem(uint8 lootSlot, Loot* loot)
{
    ...
    if (msg == EQUIP_ERR_OK)
    {
        ...
        //in case we wish to override this item
        this->TriggerCallbackFunc(CALLBACK_TYPE_LOOT_ITEM, &CP_ITEM_LOOTED(this, newitem));
...
uint32 Player::CalculateTalentsPoints() const
{
    ...
    //if (getClass() != CLASS_DEATH_KNIGHT || GetMapId() != 609) //should be uncommented on a blizzlike server
    int32 ret = int32(base_talent * sWorld->getRate(RATE_TALENT));
    int64 ExtraTalents = *((const_cast<Player *>(this))->GetCreateIn64Extension(OE_PLAYER_EXTRA_TALENT_POINTS));
    ret += ExtraTalents;
    if (ret >= 0)
        return ret;
    return 0;
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
    ...
    TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_FALL_LAND, &CP_FALL_INFO(this, m_lastFallZ, movementInfo.fallTime));
