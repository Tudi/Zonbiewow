...
void WorldSession::HandleCancelCastOpcode(WorldPacket& recvPacket)
{
    ...
    {
        Spell *cursp = _player->GetCurrentSpell(CURRENT_GENERIC_SPELL);
        if (cursp != NULL && (cursp->GetTriggerFlags() & TRIGGERED_IGNORE_MOVE_CHECK) == 0) // getextension
...
void WorldSession::HandleCancelChanneling(WorldPacket& recvData)
{
    ...
    Spell *cursp = mover->GetCurrentSpell(CURRENT_CHANNELED_SPELL);
    if(cursp != NULL && ( cursp->GetTriggerFlags() & TRIGGERED_IGNORE_MOVE_CHECK ) == 0 ) // getextension
