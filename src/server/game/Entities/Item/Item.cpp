...
{
    //do not save temporary items
    if (GetExtension<bool>(OE_ITEM_IS_TEMPORARY) != NULL)
        return;

    TriggerCallbackFunctions(CALLBACK_TYPE_SAVE_ITEM, &CP_ITEM_SAVED(this));
...
{
    // server scripts might want to delete this item also
    if(state == ITEM_REMOVED)
        TriggerCallbackFunctions(CALLBACK_TYPE_DELETE_ITEM, &CP_ITEM_SAVED(this));
...
void RemoveItemFromUpdateQueueOf(Item* item, Player* player)
{
    ...
    //this bugs out it seems for items that never get saved
    if(player->m_itemUpdateQueue.size() > item->uQueuePos && player->m_itemUpdateQueue[item->uQueuePos] == item)
        player->m_itemUpdateQueue[item->uQueuePos] = nullptr;
    //this is not good, seems like item queue is corrupted, we are removing an item that is queued to get updated, but player queue position is different than what item knows about
    else
    {
        for (int32 i = (int32)player->m_itemUpdateQueue.size() - 1; i >= 0; i--)
            if (player->m_itemUpdateQueue[i] == item)
                player->m_itemUpdateQueue[i] = nullptr;
    }
