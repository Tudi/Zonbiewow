    ...
    _zoneEntry(nullptr)
{
    ...
    //deny ownership for global channel
    std::string LowerName = name;
    std::transform(LowerName.begin(), LowerName.end(), LowerName.begin(), ::tolower);
    if (LowerName == "global")
    {
        _ownershipEnabled = false;
...
/* Ranks */
#define PLAYER          "|cffFFFFFF[Player]|r"
#define BRONZE          "|TInterface\\icons\\Inv_misc_coin_19.png:15|t" //"|cffFF8000[Bronze]|r"
#define SILVER          "|TInterface\\icons\\Inv_misc_coin_18.png:15|t" //"|cff999999[Silver]|r"
#define GOLD            "|TInterface\\icons\\Inv_misc_coin_17.png:15|t" //"|cffFFBF40[Gold]|r"
#define PLATINUM        "|TInterface\\icons\\Inv_misc_coin_08.png:15|t" //"|cff8080BF[Platinum]|r"
#define DIAMOND         "|TInterface\\icons\\Inv_misc_coin_10.png:15|t" //"|cff00BFFF[Diamond]|r"
#define DEVELOPER       "|cffFFA500[Developer]|r"
#define HELPER          "|cffbbbbbb[Helper]|r"
#define ASSISTANT       "|cffADD8E6[Assistant]|r"
#define MODERATOR       "|cff008000[Mod]|r"
#define GAMEMASTER      "|cff4169E1[GM]|r"
#define HEADGM          "|cff4169E1[Head GM]|r"
#define ADMINISTRATOR   "|cffFFFF00[Admin]|r"
#define OWNER           "|cffDA70D6[Owner]|r"
...
void Channel::Say(ObjectGuid guid, std::string const& what, uint32 lang) const
{
    ...
    std::string what2 = what;
    std::string memberhip = "";
    if (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
    {
        lang = LANG_UNIVERSAL;
        if (_channelName == "global" || _channelName == "Global")
        {
            ...
            if (player)
            {
                ...
                uint32 securityLevel = player->GetSession()->GetSecurity();
                     if (securityLevel == SEC_BRONZE) memberhip = std::string(BRONZE);
                else if (securityLevel == SEC_SILVER) memberhip = std::string(SILVER);
                else if (securityLevel == SEC_GOLD) memberhip = std::string(GOLD);
                else if (securityLevel == SEC_PLATINUM) memberhip = std::string(PLATINUM);
                else if (securityLevel == SEC_DIAMOND) memberhip = std::string(DIAMOND);
                else if (securityLevel == SEC_DEVELOPER) memberhip = std::string(DEVELOPER);
                else if (securityLevel == SEC_HELPER) memberhip = std::string(HELPER);
                else if (securityLevel == SEC_ASSISTANT) memberhip = std::string(ASSISTANT);
                else if (securityLevel == SEC_MODERATOR) memberhip = std::string(MODERATOR);
                else if (securityLevel == SEC_GAMEMASTER) memberhip = std::string(GAMEMASTER);
                else if (securityLevel == SEC_HEAD_GM) memberhip = std::string(HEADGM);
                else if (securityLevel == SEC_ADMINISTRATOR) memberhip = std::string(ADMINISTRATOR);
                else if (securityLevel == SEC_OWNER) memberhip = std::string(OWNER);
                else memberhip = std::string(PLAYER);
                
                if (player->Hardcore())
                    memberhip += std::string("|cffFF0000[HardCore]|r ");
                else if(player->Ironman())
                    memberhip += std::string("|cffD63931[IronMan]|r ");
                else
                    memberhip += std::string("|cffFFFFFF[Normal]|r ");

                if (player->GetTeam() == ALLIANCE)
                    what2 = "|TInterface\\icons\\Inv_Misc_Tournaments_banner_Human.png:15|t" + memberhip + what2;
                else
                    what2 = "|TInterface\\icons\\Inv_Misc_Tournaments_banner_Orc.png:15|t" + memberhip + what2;
            }
        }
    }
    ...
    auto builder = [&](WorldPacket& data, LocaleConstant locale)
    {
        ...
        if (Player* player = ObjectAccessor::FindConnectedPlayer(guid))
            ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang), player, player, what2, 0, GetName(localeIdx));
        else
            ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang), guid, guid, what2, 0, "", "", 0, false, GetName(localeIdx));
    };
...
void Channel::SayOfflinePlayer(ObjectGuid guid, std::string const& what) const
{
    if (what.empty())
        return;
