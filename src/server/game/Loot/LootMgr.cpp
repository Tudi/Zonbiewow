...
class LootTemplate::LootGroup                               // A set of loot definitions for items (refs are not allowed)
{
    public:
        ...
        void Process(Loot& loot, uint16 lootMode, Player *lootOwner) const;    // Rolls an item from the group (if any) and adds the item to the loot
    ...
    private:
        ...
        LootStoreItem const* Roll(Loot& loot, uint16 lootMode, Player *lootOwner) const;   // Rolls an item from the group, returns NULL if all miss their chances
...
bool LootStoreItem::Roll(bool rate, Player* lootOwner) const
{
    float LocalChance = chance;

    ItemTemplate const* pProto = sObjectMgr->GetItemTemplate(itemid);

    CP_LOOT_ROLL_CHANCE rlc(&LocalChance, lootOwner, pProto);
    TriggerCallbackFunctions(CALLBACK_TYPE_LOOT_ITEM_ROOL, &rlc);
    if(lootOwner)
        lootOwner->TriggerCallbackFunc(CALLBACK_TYPE_LOOT_ITEM_ROOL, &rlc);

    if (LocalChance >= 100.0f)
        return true;

    if (reference > 0)                                   // reference case
        return roll_chance_f(LocalChance* (rate ? sWorld->getRate(RATE_DROP_ITEM_REFERENCED) : 1.0f));

    float qualityModifier = pProto && rate ? sWorld->getRate(qualityToRate[pProto->Quality]) : 1.0f;

    return roll_chance_f(LocalChance*qualityModifier);
}
...
LootStoreItem const* LootTemplate::LootGroup::Roll(Loot& loot, uint16 lootMode, Player *lootOwner) const
{
    ...
    if (!possibleLoot.empty())                             // First explicitly chanced entries are checked
    {
        ...
        for (LootStoreItemList::const_iterator itr = possibleLoot.begin(); itr != possibleLoot.end(); ++itr)   // check each explicitly chanced entry in the template and modify its chance based on quality.
        {
            ...
            float LocalChance = item->chance;
            CP_LOOT_ROLL_CHANCE rlc(&LocalChance, lootOwner, sObjectMgr->GetItemTemplate(item->itemid));
            TriggerCallbackFunctions(CALLBACK_TYPE_LOOT_ITEM_ROOL, &rlc);
            if(lootOwner)
                lootOwner->TriggerCallbackFunc(CALLBACK_TYPE_LOOT_ITEM_ROOL, &rlc);
    ...
    {
        LootStoreItem* item = Trinity::Containers::SelectRandomContainerElement(possibleLoot);

        //check if this loot is filtered by any script
        float LocalChance = 100;
        ...
        if (LocalChance == 0.0f)
            return nullptr;
        //avoid abusing always getting boss items even if instance is lower than normal difficulty
        if(LocalChance < (float)rand_chance())
            return nullptr;

        //loot is not filtered, means we have 100% chance to roll for it
        return item;
    }
...
void LootTemplate::LootGroup::Process(Loot& loot, uint16 lootMode, Player *lootOwner) const
{
    if (LootStoreItem const* item = Roll(loot, lootMode, lootOwner))
        loot.AddItem(*item);
}
...
void LootTemplate::Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId, Player* lootOwner) const
{
    if (groupId)                                            // Group reference uses own processing of the group
    {
        ...
        Groups[groupId - 1]->Process(loot, lootMode, lootOwner);
        return;
    }
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        if (item->reference > 0)                            // References processing
        {
            ...
            int32 maxcount = uint32(float(item->maxcount) * sWorld->getRate(RATE_DROP_ITEM_REFERENCED_AMOUNT));
            TriggerCallbackFunctions(CALLBACK_TYPE_LOOT_ITEM_GROUP_ROLL, &CP_INT32_PARAM(&maxcount, lootOwner));
            for (int32 loop = 0; loop < maxcount; ++loop)      // Ref multiplicator
    ...
    for (LootGroups::const_iterator i = Groups.begin(); i != Groups.end(); ++i)
        if (LootGroup* group = *i)
            group->Process(loot, lootMode, lootOwner);
}
