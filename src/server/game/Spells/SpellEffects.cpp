            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_MINIPET:
                {
                    ...
                    if (unitCaster->ToPlayer())
                        TriggerCallbackFunctions(CALLBACK_TYPE_PLAYER_SUMMON_MINNION, &CP_CREATURE_INTERRACT(unitCaster->ToPlayer(), summon));
...
void Spell::EffectTameCreature(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitCaster->ToPlayer())
    {
        Pet *TempPet = unitCaster->ToPlayer()->GetPet();
        if(TempPet != NULL)
            unitCaster->ToPlayer()->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_SUMMON_GUARDIAN, &CP_CREATURE_INTERRACT(unitCaster->ToPlayer(), TempPet));
    }
}
...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
    ...
    Pet *TempPet = owner->GetPet();
    if (owner->ToPlayer())
        owner->ToPlayer()->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_SUMMON_GUARDIAN, &CP_CREATURE_INTERRACT(unitCaster->ToPlayer(), TempPet));
    if (!pet)
        return;
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians)
{
    ...
    //no need to crash the server
    if (unitCaster->m_Controlled.size() > 30)
        return;
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        if (unitCaster->ToPlayer())
            unitCaster->ToPlayer()->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_SUMMON_GUARDIAN, &CP_CREATURE_INTERRACT(unitCaster->ToPlayer(), summon));
