            ...
            if (!maxguid.IsEmpty())
            {
                ...
                if (player && player->GetSession())
                {
                    ...
                    if (msg == EQUIP_ERR_OK)
                    {
                        ...
                        Item *it = player->StoreNewItem(dest, roll->itemid, true, item->randomPropertyId, item->GetAllowedLooters());
                        player->TriggerCallbackFunc(CALLBACK_TYPE_LOOT_ITEM, &CP_ITEM_LOOTED(player, it));
                    }
                    else
                    {
