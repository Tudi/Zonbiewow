...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    {
#ifdef _DEBUG
        if (victim->GetExtension<int64>(OE_UNIT_DISABLE_DEATH_WHILE_TESTING) != NULL)
        {
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit, int32 TickCount)
{
    if (damage < 0)
        return;
    ...
    {
        if( uint32 *ChangeSchools = damageInfo->attacker->GetExtension<uint32>(OE_PLAYER_CHANGE_DAMGE_SCHOOL_MASK))
            damageInfo->schoolMask |= ChangeSchools[0];
    }
    ...
    if (!spellInfo->HasAttribute(SPELL_ATTR4_FIXED_DAMAGE))
    {
        ...
        switch (spellInfo->DmgClass)
        {
            ...
            case SPELL_DAMAGE_CLASS_MAGIC:
            {
                ...
                {
                    damageInfo->HitInfo |= SPELL_HIT_TYPE_CRIT;
                    CritBonus = Unit::SpellCriticalDamageBonus(this, spellInfo, damage, victim);
                    damage += CritBonus;
                }
    ...
    CP_DMG_DONE_RECEIVED CBD(damageInfo->attacker, damageInfo->target, damage - CritBonus, TickCount, spellInfo, attackType, CritBonus);
    TriggerCallbackFunctions(CALLBACK_TYPE_DMG_DONE, &CBD);
    if (damageInfo->attacker)
        damageInfo->attacker->TriggerCallbackFunc(CALLBACK_TYPE_DMG_DONE, &CBD);
    if (damageInfo->target)
        damageInfo->target->TriggerCallbackFunc(CALLBACK_TYPE_DMG_RECEIVED, &CBD);
...
void Unit::CalculateMeleeDamage(Unit* victim, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    {
        if (uint32 *ChangeSchools = damageInfo->Attacker->GetExtension<uint32>(OE_PLAYER_CHANGE_DAMGE_SCHOOL_MASK))
            damageInfo->Damages[0].DamageSchoolMask |= ChangeSchools[0];
    }
    ...
    switch (damageInfo->HitOutCome)
    {
        ...
        case MELEE_HIT_CRIT:
        {
            ...
            // Crit bonus calc
            int32 DamageBefore = damageInfo->Damages[0].Damage;
            for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
            {
            ...
            CritDamage = damageInfo->Damages[0].Damage - DamageBefore;
            break;
        }
        case MELEE_HIT_PARRY:
    ...
    CP_DMG_DONE_RECEIVED CBD(damageInfo->Attacker, damageInfo->Target, damageInfo->Damages[0].Damage - CritDamage, 1, NULL, attackType, CritDamage);
    TriggerCallbackFunctions(CALLBACK_TYPE_DMG_DONE, &CBD);
    if (damageInfo->Attacker)
        damageInfo->Attacker->TriggerCallbackFunc(CALLBACK_TYPE_DMG_DONE, &CBD);
    if (damageInfo->Target)
        damageInfo->Target->TriggerCallbackFunc(CALLBACK_TYPE_DMG_RECEIVED, &CBD);
    TriggerCallbackFunctions(CALLBACK_TYPE_DMG_RECEIVED, &CBD);
    damageInfo->Damages[0].Damage = CBD.GetDamage();
...
void Unit::SetCurrentCastSpell(Spell* pSpell)
{
    ...
    switch (CSpellType)
    {
        case CURRENT_GENERIC_SPELL:
        {
            ...
            if ((pSpell->GetTriggerFlags() & TRIGGERED_IGNORE_CAST_IN_PROGRESS) == 0) // getextension
...
void Unit::RemoveAurasWithInterruptFlags(uint32 flag, uint32 except)
{
    ...
    if((flag & (AURA_INTERRUPT_FLAG_CAST| AURA_INTERRUPT_FLAG_SPELL_ATTACK))==0)
        if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL])
            if (spell->getState() == SPELL_STATE_CASTING
...
/*static*/ void Unit::DealHeal(HealInfo& healInfo)
{
    ...
    CP_DMG_DONE_RECEIVED CP(healer, victim, addhealth, healInfo.TickCount, healInfo.GetSpellInfo(), MAX_ATTACK, 0);
    TriggerCallbackFunctions(CALLBACK_TYPE_HEAL_DONE, &CP);
    if (healer != NULL)
        healer->TriggerCallbackFunc(CALLBACK_TYPE_HEAL_DONE, &CP);
    if (victim != NULL)
        victim->TriggerCallbackFunc(CALLBACK_TYPE_HEAL_RECEIVED, &CP);
    addhealth = CP.GetDamage();
...
/*static*/ void Unit::Kill(Unit* attacker, Unit* victim, bool durabilityLoss /*= true*/)
{
    ...
    if (isRewardAllowed && player && player != victim)
    {
        ...
        if (creature)
        {
            ...
            if (creature->GetLootMode() > 0)
                loot->generateMoneyLoot(creature->GetCreatureTemplate()->mingold, creature->GetCreatureTemplate()->maxgold, looter);
    ...
    if (attacker)
        if (Player* killerPlayer = attacker->GetCharmerOrOwnerPlayerOrPlayerItself())
        {
            ...
            killerPlayer->TriggerCallbackFunc(CALLBACK_TYPE_TARGET_KILL, &CP_DMG_DONE_RECEIVED(killerPlayer, victim, 1, 1, NULL, MAX_ATTACK, 0));
        }
    ...
    if (attacker)
    {
        if (Player* killerPlr = attacker->ToPlayer())
        {
            ...
            else if (Creature* killedCre = victim->ToCreature())
            {
                ...
                killerPlr->TriggerCallbackFunc(CALLBACK_TYPE_PLAYER_KILL_CREATURE,&CP_CREATURE_INTERRACT(killerPlr, killedCre));
            }
        }
