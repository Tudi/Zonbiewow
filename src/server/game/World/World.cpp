...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_float_configs[CONFIG_THREAT_RADIUS] = sConfigMgr->GetFloatDefault("ThreatRadius", 60.0f); //THIS IS NOT USED ANYWHERE?
    ...
    m_float_configs[CONFIG_HUNGER_GAMES_RADIUS] = sConfigMgr->GetFloatDefault("HG.Radius", 100.0f);
    ...
    m_int_configs[CONFIG_INSTANCE_MAX_DIFFICULTY] = sConfigMgr->GetIntDefault("MaxInstanceDifficulty", 1000);
    m_int_configs[CONFIG_RANDOMIZED_INSTANCE_ITEMS] = sConfigMgr->GetIntDefault("RandomizeInstanceItemDrops", 0);
    m_int_configs[CONFIG_PARAGON_LEVELS] = sConfigMgr->GetIntDefault("EnableParagonLevels", 0);
...
/// Remove a given session
Player *World::GetOnlinePlayer(ObjectGuid GUID)
{
    for (SessionMap::iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        if (!plr || itr->second->isLogingOut() || plr->GetGUID() != GUID)
            continue;
