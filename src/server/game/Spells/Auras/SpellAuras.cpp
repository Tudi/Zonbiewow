...
void AuraApplication::BuildUpdatePacket(ByteBuffer& data, bool remove) const
{
    ...
    data << uint8(aura->GetCharges() == 0 ? aura->GetStackAmount() : aura->GetCharges());
...
bool Aura::ModStackAmount(int32 num, AuraRemoveMode removeMode /*= AURA_REMOVE_BY_DEFAULT*/, bool resetPeriodicTimer /*= true*/)
{
    ...
    int32 SpellInfoStackAmount = m_spellInfo->StackAmount;
    if (GetCaster() != NULL && m_spellInfo->IsPassive() == false && GetMaxDuration() != -1)
    {
        if (int32 *ExtraStacks = GetCaster()->GetExtension<int32>(OE_PLAYER_MAX_AURA_STACKS))
        {
            if (SpellInfoStackAmount == 0)
                SpellInfoStackAmount = 1;
            SpellInfoStackAmount += ExtraStacks[0];
        }
    }
    // limit the stack amount (only on stack increase, stack amount may be changed manually)
    if ((num > 0) && (stackAmount > int32(SpellInfoStackAmount)))
    {
        // not stackable aura - set stack amount to 1
        if (!SpellInfoStackAmount)
            stackAmount = 1;
        else
            stackAmount = SpellInfoStackAmount;
    }
...
bool Aura::CanStackWith(Aura const* existingAura) const
{
    ...
    if (m_spellInfo->IsRankOf(existingSpellInfo))
    {
        ...
        {
            CP_AURA_CAN_STACK cbp(Owner, existingAura);
            Owner->TriggerCallbackFunc(CALLBACK_TYPE_SAME_AURA_CAN_STACK,&cbp);
            return cbp.CanStack;
        }
        return false;
    }
